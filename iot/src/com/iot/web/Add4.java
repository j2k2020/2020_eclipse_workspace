...
@WebServlet("/Add4") //application공유 공간
		...
		ServletContext application=request.getServletContext(); //저장공간!!!	
		 ...
		 if(operator_.equals("=")) {
			 ...
			 int result=0;
			   ...
			   application.removeAttribute("operator");
			   
		 }else {
			  //이전 저장되어 있는 오페리이트를 가져와 계산만 하는 작업 그리고 결과를 임시 저장
			  if(application.getAttribute("operator").equals("+")) {
				 application.setAttribute("result", (Integer)application.getAttribute("result")+value);//application앞에 (Integer)를 넣어서 정수형으로 형변화하여 value랑 같은 형으로 변경해줘야 됨.
			  } else if(application.getAttribute("operator").equals("-")) {
